import sys
sys.path.append('..')
from experiment.param_strong import par_list_strong,p_ams
...
time = np.arange(1,max_regret.shape[0]+1)
s_regret = max_regret/time
np.save('../result/SC_ECDAMSGrad_SR.npy', s_regret)
np.save('../result/SC_ECDAMSGrad_Bits.npy', m.avg_trans_bit)
...
plt.savefig('../figures/Four_SC_Time.pdf')
...
#plt.tight_layout()
plt.subplots_adjust(left=0.15)
plt.savefig('../figures/Four_SC_Bits.pdf')
