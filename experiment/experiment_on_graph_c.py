import sys
sys.path.append('..')
from experiment.param_on_graph_c import meta_par_list_graph
...
for meta  in meta_par_list_graph:
    for p in meta:
        ...
        #np.save('../result_appendix/SC_{}_SR.npy'.format(p.kwargs['label']), s_regret)
        #np.save('../result_appendix/SC_{}_Bits.npy'.format(p.kwargs['label']), m.avg_trans_bit)
        ...
        plt.savefig('../figures_appendix/C_GraphType_Time.pdf')
        ...
        plt.grid(True)
        plt.subplots_adjust(left=0.15)
        plt.savefig('../figures_appendix/C_GraphType_Bits.pdf')
    plt.show()
