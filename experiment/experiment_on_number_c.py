...
from experiment.param_on_number_c import meta_par_list_graph
...
for meta in meta_par_list_graph:
    ...
    for p in meta:
        ...
        res[0,i] = p.agent_num
        res[1,i] = ( max_regret[-1] / (p.epoch)  - (off[-1] * 785 / p.epoch) ) / p.agent_num
        i+=1
        np.save('../result/C_{}_res.npy'.format(p.kwargs['label']), res)
...
plt.savefig('../figures/C_Number_Time.pdf')
plt.show()
